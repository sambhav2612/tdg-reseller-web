...
/*----------------------------------------------------------------------------------------------------------------------
 Fixed Departures  Handler
----------------------------------------------------------------------------------------------------------------------*/


  $.ajax({
    url: TDG.urls.GET_DEPARTURES + '?plan=' + TDG.plan,
    //url:'http://localhost:3000/license/test.traveldglobe.in/get-departures?plan=' + TDG.plan,

    method: 'get',
    success: function (response) {

        $(".date-container").html(response);
    },
    error: function (err) {
    }
  });
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  var handleBookingBtn = function() {
    ...
    var date = $('.js-startDate:checked').val() || $('.js-startDate').val() ||'';
    ...
    $.ajax({
      ...
      method: 'post',
      data: {product: product, is_tour: true},
      success: function (response) {
